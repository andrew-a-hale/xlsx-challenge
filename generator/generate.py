...
def write_table(
    ...
    data = df.fetchall()
    for i, (_, header) in enumerate(UNIT_SCHEMAS[unit]):
        ws.cell(row_offset, i + 1, header)

    row_offset += 1
    for r, _ in enumerate(data):
        tmp = r + row_offset
        for c, (col, _) in enumerate(UNIT_SCHEMAS[unit]):
            if col == "row_id":
...
def create_data(source: str, out: str):
    ...
    outfile = open(os.path.join(PATH, tmp), mode="w")
...
def create_zip(out: str):
    ...
    zip_filename = os.path.join(PARENT, f"{outfile}.zip")
